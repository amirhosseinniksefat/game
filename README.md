...
6. [نحوه بروزرسانی (Update) پروژه در لوکال و سرور](#6-نحوه-بروزرسانی-update-پروژه-در-لوکال-و-سرور)
...
## 🔄 6. نحوه بروزرسانی (Update) پروژه در لوکال و سرور

پس از اینکه فایل‌ها را ویرایش کردید یا تغییرات جدید را از گیت‌هاب دریافت نمودید، به روش‌های زیر می‌توانید پروژه را بروزرسانی کنید:

### ۱. بروزرسانی در محیط لوکال (Windows, Linux, Termux):
1. اگر پروژه در حال اجراست، با کلیدهای ترکیب `Ctrl + C` آن را در ترمینال متوقف کنید.
2. آخرین کدهای جدید را دریافت کنید (در صورت استفاده از گیت):
   ```bash
   git pull origin main
   ```
3. در صورت افزودن پکیج‌های جدید، آن‌ها را نصب کنید:
   ```bash
   npm install
   ```
4. پروژه را مجدداً اجرا نمایید:
   ```bash
   npm run dev
   ```
...
### ۲. بروزرسانی روی سرور واقعی (VPS با PM2):
هنگام اعمال تغییرات جدید روی سرور پروداکشن، حتماً باید پروژه بیلد مجدد شود و PM2 ری‌استارت گردد:

1. وارد پوشه پروژه در سرور شوید:
   ```bash
   cd path/to/your/project
   ```
2. آخرین تغییرات کدهای خود را دانلود کنید:
   ```bash
   git pull origin main
   ```
3. کتابخانه‌های احتمالی جدید را نصب کنید:
   ```bash
   npm install
   ```
4. **ساخت بیلد جدید پروداکشن (الزامی):**
   ```bash
   npm run build
   ```
5. **ری‌استارت کردن برنامه با PM2:**
   ```bash
   pm2 restart ludo-game
   ```
   *(حالا تغییرات جدید بدون قطعی سرور روی نسخه پروداکشن اعمال می‌شود).*

---

### ۳. بروزرسانی روی سرور با داکر (Docker):
اگر پروژه شما با داکر اجرا می‌شود، مراحل زیر را طی کنید:

1. کدهای جدید را بگیرید:
   ```bash
   git pull origin main
   ```
2. ایمیج داکر را مجدداً بیلد کنید:
   ```bash
   docker build -t ludo-game .
   ```
3. کانتینر قبلی را متوقف و حذف کرده و کانتینر جدید را روشن کنید:
   ```bash
   docker stop ludo-game-app
   docker rm ludo-game-app
   docker run -d -p 3000:3000 --env-file .env --name ludo-game-app ludo-game
   ```
